...
createAntaresStudyFromIfScenario <- function(study_name, scenario_number) {
  ...
  # ...do we really need nominal capacities when we have the timeseries? honestly?
  # Paul confirmed no !
  # # Dynamically create the column name based on scenario_number
  # scenario_column <- paste0("if_2060_", scenario_number)
  # 
  # # Filter for the Wind row and select the correct scenario column
  # wind_value <- volumes_MATER_tbl %>%
  #   filter(if_technology_type == "Wind") %>%
  #   pull(!!sym(scenario_column))
  ...
  ################################## PV IMPORT #################################
    ...
    for (node in nodes) {
      ...
      tryCatch({
        ...
        # red flag de fou : ça s'active pour tous les trucs alors que y a bcp de pays sans CSP
        # ça n'a pas l'air d'invalider le délire, en fait ça crée des TS vide.
        # mais bon. ça prend du temps pour rien un peu.
        logFull(msg)
      }, error = function(e) {
  ...
  # pour peu que j'enlève les mainteannces ça élimine mon souci de "euh j'ai bien recalculé les volumes ??"
  # et après oui le PV et l'éolien faut faire gaffe à ce qu'il n'y ait pas d'absents dans PLEXOS.
    
    # En fait on reboucle sur le constat~: peut-être qu'un noeud monde c'est effectivement basé.
